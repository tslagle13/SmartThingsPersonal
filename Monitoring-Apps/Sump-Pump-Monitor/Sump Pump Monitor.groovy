...
preferences {
    ...
    section("Danger Zone?") {
		input "frequency", "number", title: "If it runs twice in X minutes send and alert...", description: "Minutes", required: true
        input "frequencyLong", "number", title: "If it hasn't run in X hours send and alert...", description: "Hours", required: true, defaultValue:5
        input "interval", "number", title:"How often should I check? (in minutes)", defaultValue:15
...
def checkFrequencyLong(evt){
log.debug("running check sump")
def lastTime = state[frequencyKeyAccelration(evt)]
log.info(now() - lastTime)
log.info(frequencyLong * 3600000)
if (lastTime == null) {
	state[frequencyKeyAccelration(evt)] = now()
}
else if (now() - lastTime >= frequencyLong * 3600000) {
log.debug("Last time valid")
def timePassed = now() - lastTime
def timePassedFix = timePassed / 3600000
def timePassedRound = Math.round(timePassedFix.toDouble()) + (unit ?: "")
state[frequencyKeyAccelration(evt)] = now()
def msg = messageText ?: "Alert: Sump pump has not ran in the last ${timePassedRound} minutes."
